...
カバー画像の列数を自動計算します。HerdrのKitty graphicsは実験的機能のため、
正確な縦横比が必要な場合は`Ctrl+t`または`Cmd+t`で素のWezTermタブを開き、
Herdrを介さず`spotify_player`を起動します。
...
NeovimはNixvim packageへ移行したため、`setup.sh`ではリンクしません。旧LazyVim
構成へのリンクが残っている場合は、`readlink ~/.config/nvim`がこのリポジトリの
削除済み`nvim`ディレクトリを示すことを確認してから、`unlink ~/.config/nvim`で
リンクだけを解除します。並行検証には`nix run .#nixvim`を使用します。
